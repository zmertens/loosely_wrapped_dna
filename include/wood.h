...
struct wood_node {
    ...
    wood_node(const std::string &lab, float len = 0.0f)
        : label(lab), length(len), anc(0), right(0) {}
    wood_node(const std::string &lab, const boost::optional<float> &len)
        : label(lab), length(len.get_value_or(0.0f)), anc(0), right(0) {}
    inline bool unlabeled() const { return (label.empty() || label[0] == '('); }
    inline bool terminal() const { return (right == 0); }
    inline bool one_child() const { return (right == 1); }
    inline bool two_child() const { return (right > 1); }
};
...
class wood {
   public:
    ...
    inline bool has_desc(const std::string &d) const {
        return desc_names.count(d) != 0;
    }
    inline const std::set<std::string> &desc_labels() const {
        return desc_names;
    }
    ...
    bool parse(const std::string &str) { return parse(str.begin(), str.end()); }
    ...
    bool autolabel() {
        ...
        desc_names.clear();
        for(data_type::reverse_iterator it = _data.rbegin(); it != _data.rend();
            ++it) {
            ...
            if(!desc_names.insert(it->label).second)
                return DAWG_ERROR("invalid tree; node label '"
                                  << it->label
    ...
    template <typename Iterator>
    static std::reverse_iterator<Iterator> get_left(
        std::reverse_iterator<Iterator> me) {
        std::advance(me, -1);
        return me;
    }
    template <typename Iterator>
    static std::reverse_iterator<Iterator> get_right(
        std::reverse_iterator<Iterator> me) {
        std::advance(me, -static_cast<int>(me->right));
        return me;
    }
    template <typename Iterator>
    static std::reverse_iterator<Iterator> get_anc(
        std::reverse_iterator<Iterator> me) {
        std::advance(me, static_cast<int>(me->anc));
        return me;
    }
