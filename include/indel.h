...
#include "loosely_wrapped_dna/include/specfunc.h"
...
class indel_model {
   public:
    ...
    bool create(It1 first_n, It1 last_n, It2 first_r, It2 last_r, It3 first_p,
                It3 last_p, unsigned int max_size) {
        static std::string name_keys[] = {
            std::string("user"),      std::string("geom"),
            std::string("zeta"),      std::string("zipf"),
            std::string("power-law"), std::string("yule-simon"),
            std::string("lavalette")};
        ...
        for(It2 it = first_r; it != last_r; ++it) {
            if(*it < 0.0)
                return DAWG_ERROR("invalid indel model; rate '"
                                  << *it << "' must be positive");
            total_rate += *it;
        }
        if(total_rate == 0.0) {
        ...
        for(It2 it = first_r; it != last_r; ++it) {
            ...
            case 0:  // user model
                okay =
                    create_user(fraction, first_p, last_p, max_size, mix_dist);
                break;
            case 1:  // geometric model
                okay =
                    create_geo(fraction, first_p, last_p, max_size, mix_dist);
                break;
            case 2:  // zeta power-law model
            case 3:
            case 4:
                okay =
                    create_zeta(fraction, first_p, last_p, max_size, mix_dist);
                break;
            case 5:  // yule-simon model
                okay =
                    create_yule(fraction, first_p, last_p, max_size, mix_dist);
                break;
            case 6:  // lavalette model
                okay = create_lavalette(fraction, first_p, last_p, max_size,
                                        mix_dist);
                break;
            default:
                return DAWG_ERROR("Invalid indel model; no model named '"
                                  << *itn << "'");
            };
            if(!okay) return DAWG_ERROR("indel model creation failed");
            // cycle "name" as needed
            if(++itn == last_n) itn = first_n;
        }
        // Calculate mean
        double m = 0.0, w = 0.0;
        std::vector<double> upstream_dist(mix_dist.size(), 0.0);
        for(boost::uint32_t x =
                static_cast<boost::uint32_t>(mix_dist.size() - 1);
            x != 0; --x) {
            m += mix_dist[x] * x;
            w += mix_dist[x];
    ...
    boost::uint32_t operator()(random_t &m) const { return sample(m.u64()); }

    boost::uint32_t sample_upstream_overlap(random_t &m) const {
        return sample_upstream(m.u64());
    }
   ...
   private:
    ...
    inline bool create_geo(double f, It &first, It last, unsigned int max_size,
        ...
        if(p >= 1.0) p = 1.0 / p;
        if(p <= 0.0)
            return DAWG_ERROR("Invalid indel model; geo parameter '"
                              << p << "' must be positive.");
        double d = p;
    ...
    inline bool create_zeta(double f, It &first, It last, unsigned int max_size,
        ...
        double z = *first++;
        if(z <= 1.0)
            return DAWG_ERROR("Invalid indel model; zeta parameter '"
                              << z << "' must be > 1");
        double zz = zeta(z);
        for(boost::uint32_t n = 1; n <= max_size; ++n) {
    ...
    inline bool create_yule(double f, It &first, It last, unsigned int max_size,
        ...
        double z = *first++;
        if(z <= 1.0)
            return DAWG_ERROR("Invalid indel model; yule parameter '"
                              << z << "' must be > 1");
        for(boost::uint32_t n = 1; n <= max_size; ++n) {
    ...
    template <typename It>
    inline bool create_lavalette(double f, It &first, It last,
                                 unsigned int max_size,
                                 std::vector<double> &mix_dist) {
        if(first == last)
            return DAWG_ERROR(
                "Invalid indel model; lavalette requires 2 parameter");
        double z = *first++;
        if(first == last)
            return DAWG_ERROR(
                "Invalid indel model; lavalette requires 2 parameter");
        double dm = *first++;
        if(z <= 1.0)
            return DAWG_ERROR("Invalid indel model; lavalette slope '"
                              << z << "' must be > 1");
        if(dm <= 1.0)
            return DAWG_ERROR("Invalid indel model; lavalette max '"
                              << dm << "' must be > 1");
    ...
    inline bool create_user(double f, It &first, It last, unsigned int max_size,
        ...
        if(first == last)
            return DAWG_ERROR(
                "Invalid indel model; no parameters for user model.");
        // sum up parameters
        for(; it != last && *it >= 0.0; ++it) d += *it;
