...
struct trick_grammar
    : qi::grammar<Iterator, details::trick_raw_type(), skip_type> {
    trick_grammar() : trick_grammar::base_type(start) {
        ...
        trick_string =
            qqquoted_string | quoted_string | tree_string | bare_string;
    ...
    qi::rule<Iterator, details::section_header_type(), skip_type>
        section_header;
    ...
    qi::rule<Iterator, details::subsection_body_type(), skip_type>
        subsection_body;
...
bool trick::parse(Iterator first, Iterator last) {
    ...
    for(details::trick_raw_type::const_iterator it = pyle.begin();
        ...
        if(!sec.first.first.empty()) {
            ...
            psec->inherits =
                sec.first.second.empty() ? header : sec.first.second;
        ...
        for(details::section_body_type::const_iterator ssit = body.begin();
            ...
            if(h.empty())  // if h is empty, clear subheader
                ...
                if(!subheader.empty() && !hh.empty()) subheader.append(1, '.');
                subheader.append(hh);
            } else if(starts_with(h,
                                  ".")) {  // if h begins with a single period,
                                           // append it to existing subheader
            ...
            // cycle through lines
            for(details::subsection_body_type::const_iterator lit =
                    ssit->second.begin();
                lit != ssit->second.end(); ++lit) {
                std::string hh(subheader);
                if(!hh.empty()) hh.append(1, '.');
                hh.append(lit->first);
                to_lower(hh);
