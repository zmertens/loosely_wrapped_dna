...
template <typename T, typename A>
inline void trick::section::get(const std::string& k,
...
inline void trick::section::get(const std::string& k, trick::section& r) const {
    ...
    for(last = first; last != db.end() && starts_with(last->first, k); ++last) {
        r.db.insert(r.db.end(),
                    make_pair(erase_head_copy(last->first,
...
inline void trick::section::conv(const std::string& ss, bool& r) {
    ...
    r = !(ss.empty() || iequals(ss, "false") || iequals(ss, "0") ||
          iequals(ss, "f") || iequals(ss, "off") || iequals(ss, "no"));
}

inline void trick::section::read_alias(const std::string& a,
                                       const std::string& b) {
    db_type::const_iterator it;
    // if b exists or a doesn't, stop
    if(db.find(b) != db.end() || (it = db.find(a)) == db.end()) return;
    db[b] = it->second;
}

inline void trick::section::read_aliases() {
#define XM(aname, bname) read_alias(XP(aname), XP(bname));
#include "loosely_wrapped_dna/include/aliases.xmh"
#undef XM
}
