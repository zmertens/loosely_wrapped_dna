...
class rate_model {
   public:
    ...
    bool create(const std::string& rname, It first, It last) {
        static const std::string name_keys[] = {std::string("const"),
    ...
    bool create_gamma(It first, It last) {
        if(first == last)
            return DAWG_ERROR(
                "Invalid rate model; gamma-invariant requires at least 1 "
                "parameter");
        double alpha = *first++;
        if(alpha < 0.0)
            return DAWG_ERROR(
                "Invalid rate model; first gamma-invariant parameter '"
                << alpha << "' is not >= 0.");
        double iota = 0.0;
        if(first != last) {
            iota = *first++;
            if(iota < 0.0 || iota >= 1.0)
                return DAWG_ERROR(
                    "Invalid rate model; second gamma-invariant parameter '"
                    << iota << "' is not [0,1).");
        }
        ...
        if(first != last) {
            ...
            if(sz < 1 || sz > 65535)
                return DAWG_ERROR(
                    "Invalid rate model; third gamma-invariant parameter '"
                    << sz << "' is not in [1,65535].");
        }
        bool do_median = false;
        if(first != last) do_median = (*first++ != 0.0);
        ...
        if(do_median) {
            ...
            for(int k = 0; k < sz; ++k)
                values_[k + 1] =
                    boost::math::quantile(gamma_dist, (2 * k + 1) / (2.0 * sz));
        } else {
            std::vector<double> g(sz);
            boost::math::gamma_distribution<> gamma_dist(alpha, 1.0 / alpha);
            boost::math::gamma_distribution<> gamma_dist2(alpha + 1.0,
                                                          1.0 / alpha);
        ...
        double d = 0.0;
        for(size_t k = 1; k < values_.size(); ++k) d += values_[k] * gw;
        for(size_t k = 1; k < values_.size(); ++k) values_[k] /= d;
    ...
    inline const std::string& label() const { return name_; }

    category_type operator()(fragmites::random::Random& m) const {
        return sample_(m.u64());
    }
