...
class output {
   public:
    ...
    inline bool operator()(const alignment &aln) {
        ...
        if(!do_split) out << ((rep == 0) ? block_head : block_between);
        out << block_before;
        (this->*do_op)(aln);
        out << block_after;
        if(!do_split == 0 && rep == last_rep) out << block_tail;
        ++rep;
        out.flush();
        return true;
    }
    ...
    inline void set_ostream(std::ostream &os) { p_out = &os; }
    inline void set_ostream(std::ostream *os) { p_out = os; }

    inline void set_blocks(const char *h, const char *w, const char *t,
...
bool output::set_format(T format) {
    static constexpr char format_keys[][10] = {"aln", "poo",   "fasta",
                                               "fsa", "nexus", "phylip"};
    static void (output::*format_ops[])(const alignment &aln) = {
        &output::print_aln,   &output::print_poo,   &output::print_fasta,
        &output::print_fasta, &output::print_nexus, &output::print_phylip};
