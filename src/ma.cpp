...
void dawg::ma::read_section(const trick::data_type::value_type &sec) {
    name = sec.name;
#define XM(aname, type, def, desc) sec.get(XP(aname), XV(aname));
#include "loosely_wrapped_dna/include/dawgma.xmh"
#undef XM
}
    ...
    for(trick::data_type::const_iterator secit = trk.data.begin();
        ...
        if(iit == lookup.end())
            return DAWG_ERROR("section '" << secit->inherits
                                          ...
                                          << secit->name << "')");
        // lookup section
        pair<map_t::iterator, bool> me =
            lookup.insert(make_pair(secit->name, (dawg::ma *)nullptr));
        if(!me.second)
            return DAWG_ERROR("section '" << secit->name
