...
#include "loosely_wrapped_dna/include/wood_parse.h"
...
bool dawg::matic::add_config_section(const dawg::ma &ma) {
    ...
    if(ma.root_segment >= configs.size()) configs.resize(ma.root_segment + 1);
    ...
    if(seg.empty()) {  // new segment
        if(!seg.rex.model(info->sub_mod.seq_type(), info->sub_mod.seq_code(),
                          ma.output_rna, ma.output_lowercase, ma.output_markins,
                          ma.output_keepempty))
            return DAWG_ERROR(
                "failed to create sequence type or format object.");
    } else if(!seg.rex.is_same_model(info->sub_mod.seq_type(),
                                     info->sub_mod.seq_code(), ma.output_rna,
                                     ma.output_lowercase, ma.output_markins,
                                     ma.output_keepempty)) {
        return DAWG_ERROR(
            "the sequence type or format options of a section is different "
            "than its segment.");
    }
    info->gap_base = seg.rex.gap_base();

    if(!info->ins_mod.create(ma.indel_model_ins.begin(),
                             ma.indel_model_ins.end(),
                             ma.indel_rate_ins.begin(), ma.indel_rate_ins.end(),
                             ma.indel_params_ins.begin(),
                             ma.indel_params_ins.end(), ma.indel_max_ins))
        return DAWG_ERROR("insertion model could not be created.");
    if(!info->del_mod.create(ma.indel_model_del.begin(),
                             ma.indel_model_del.end(),
                             ma.indel_rate_del.begin(), ma.indel_rate_del.end(),
                             ma.indel_params_del.begin(),
                             ma.indel_params_del.end(), ma.indel_max_del))
    ...
    for(auto &sec : seg) {
        std::set<std::string>::const_iterator it =
            has_intersection(sec->usertree.desc_labels().begin(),
                             sec->usertree.desc_labels().end(),
                             info->usertree.desc_labels().begin(),
                             info->usertree.desc_labels().end());
        if(it != info->usertree.desc_labels().end())
            return DAWG_ERROR("invalid tree; descendent '"
                              << *it << "' already exists in segment #"
                              << ma.root_segment);
    }
    ...
    for(it = seg.begin();
        it != seg.end() &&
        !info->usertree.has_desc(it->get()->usertree.root_label());
        ++it)
        /*noop*/;
...
bool dawg::matic::finalize_configuration() {
    ...
    configs.erase(remove_if(configs.begin(), configs.end(),
    ...
    for(auto &seg : configs) {
        ...
        for(auto &sec : seg) {
            ...
            it = label_union
                     .insert(label_to_index_type::value_type(
                         sec->usertree.root_label(), 0))
                     .first;
            // if this root hasn't been seen before in this segment, mark it
            if(it->second < u) {
                if(has_root)
                    return DAWG_ERROR("segment "
                                      << u - 1 << " has an extra root ( "
            ...
            for(const auto &lab : sec->usertree.desc_labels()) {
                it = label_union.insert(
                    it, label_to_index_type::value_type(lab, 0));
                it->second = u;
            }
        }
    ...
    for(auto &lab : label_union) {
        lab.second = u++;
        if(lab.first[0] != '{' && lab.first[0] != '~') aln_size = u;
    }
    if(aln_size == 0) {
...
void dawg::matic::walk(alignment &aln) {
    ...
    if(configs[0][0]->gap_overlap) {
        ...
        for(const auto &sec : configs[0]) {
            for(wood::data_type::size_type u = 1;
                u < sec->usertree.data().size(); ++u) {
                ...
                sec->evolve_upstream(sd.seq, sd.indels, n.length, branch_color,
                                     maxx);
                sec->evolve(sd.seq, sd.indels, n.length, branch_color,
                            ranc.begin(), ranc.end(), maxx);
    ...
    for(const segment &seg : configs) {
        ...
        {  // create root sequence of this
            ...
            sec->rut_mod(seqs[sec->metatree[0]].seq, maxx, sec->sub_mod,
                         sec->rat_mod, branch_color);
            branch_color += dawg::residue::branch_inc;
            // if gap_overlap is false, clear the upstream buffer of all
            // sequences
            if(!sec->gap_overlap) {
                for(details::sequence_data &v : seqs) {
                    v.indels.clear();
...
void dawg::details::matic_section::evolve_upstream(
    ...
    residue::data_type branch_color, random_t &m) const {
    double dM, d;
    ...
    if(dM > DBL_EPSILON) {
        ...
        while(d < T) {
            del_up.push(
                indel_data::element(d / T, del_mod.sample_upstream_overlap(m)));
            d += m.exp(dM);
        }
    }
    ...
    while(!ins_up.empty()) {
        ...
        evolve_indels(child, indels, T, branch_color, temp.begin(), temp.end(),
                      m);
    }
...
dawg::sequence::const_iterator dawg::details::matic_section::evolve_indels(
    sequence &child, indel_data &indels, double T,
    residue::data_type branch_color, sequence::const_iterator first,
    sequence::const_iterator last, random_t &m) const {
    double f, t;
        ...
        if(!indels.del.empty()) {
            ...
            if(!indels.ins.empty()) {
                ...
                if(x <= 2 * u) {
                    ...
                    r.second -= u;
                    n.second -= u;
                    if(r.second == 0) indels.del.pop();
                    if(n.second == 0) indels.ins.pop();
                    // push the new event on the proper stack
                    if((x & 1) == 1) {
                        indels.del.push(indel_data::element(
                            tt + t * f / del_rate, del_mod(m)));
                    } else {
                        do {
                            indels.ins.push(indel_data::element(
                                tt + t * f / ins_rate, ins_mod(m)));
                            f += m.exp(t * T);
                        } while(f < ins_rate);
                    }
                } else {
                ...
                child.insert(child.end(), u,
                             residue(gap_base, 0, branch_color));
                ...
                if(x <= 2 * r.second) {
                    ...
                    if((x & 1) == 1) {
                        indels.del.push(
                            indel_data::element(t * f / del_rate, del_mod(m)));
                    } else {
                        do {
                            indels.ins.push(indel_data::element(
                                t * f / ins_rate, ins_mod(m)));
                            f += m.exp(t * T);
                        } while(f < ins_rate);
                    }
                } else {
            ...
            boost::uint32_t x = next_indel(m.exp(t * T), f, false);
            ...
            if(x <= 2 * u) {
                ...
                if((x & 1) == 1) {
                    indels.del.push(
                        indel_data::element(tt + t * f / del_rate, del_mod(m)));
                } else {
                    do {
                        indels.ins.push(indel_data::element(
                            tt + t * f / ins_rate, ins_mod(m)));
...
void dawg::details::matic_section::evolve(sequence &child, indel_data &indels,
                                          double T,
    ...
    for(;;) {
        ...
        if(d < del_rate) {
            ...
            first =
                evolve_indels(child, indels, T, branch_color, first, last, m);
            d = m.exp(T);
            continue;
        } else
            d -= del_rate;
...
void dawg::matic::align(alignment &aln, const seq_buffers_type &seqs,
    ...
    for(;;) {
        ...
        for(aligner_data &v : aln_table) {
            if(v.it == v.last) continue;  // Sequence is done
            ...
            if(uBranchN == uBranch) {
                ...
                uBranch = uBranchN;
                uState =
                    uStateQuit & ((v.it->base() == rex.gap_base()) ? 0x1 : 0x0);
            }
        }
        switch(uState & 3) {
        case 3:
        case 2:
            goto ENDFOR;  // Yes, you shouldn't use goto, except here
        case 1:           // Empty column that we want to ignore
            for(aligner_data &v : aln_table) {
                if(v.it != v.last && v.it->branch() == uBranch) ++v.it;
            }
            break;
