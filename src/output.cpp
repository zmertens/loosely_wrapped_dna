...
bool dawg::output::open(const char* file_name, unsigned int max_rep, bool split,
                        bool append, bool label) {
    ...
    if(file_name != nullptr && file_name[0] != '\0') {
        ...
        if(mid != nullptr) {
            ...
            // file.format
            format = boost::make_iterator_range(
                mid + 1, (const char*)strchr(mid + 1, '\0'));
        }
...
bool dawg::output::open_file(const char* file_name) {
    ...
    ios_base::openmode om =
        ios_base::out | (do_append ? ios_base::app : ios_base::trunc);
...
bool dawg::output::open_next() {
    ...
    generate(current_label.begin(), right_align(label_width, lit('0'))[uint_],
             rep);

    // no need to open next file
    if(!do_split) return true;
...
void dawg::output::print_poo(const alignment& aln) {
    ...
    for(const alignment::value_type& v : aln) {
        out << setw(aln.max_label_width) << v.label << '_' << current_label
            << ' ' << v.seq << endl;
    }
    out << endl;
}
...
void dawg::output::print_fasta(const alignment& aln) {
    ...
    for(const alignment::value_type& v : aln) {
        out << ">" << v.label;
        if(do_label) out << "_" << current_label;
        out << endl;
...
void dawg::output::print_aln(const alignment& aln) {
    ...
    string::size_type aln_label_width =
        do_label ? std::max(aln.max_label_width + label_width + 1,
                            string::size_type(14))
    ...
    for(; u + 60 < len; u += 60) {
        for(const alignment::value_type& v : aln) {
            ...
            if(do_label) {
                out << '_' << current_label;
                out << setw(aln_label_width -
                            (v.label.length() + label_width + 1) + 1)
                    << ' ';
            } else {
    ...
    for(const alignment::value_type& v : aln) {
        ...
        if(do_label) {
            out << '_' << current_label;
            out << setw(aln_label_width - (v.label.length() + label_width + 1) +
                        1)
                << ' ';
        } else {
            out << setw(aln_label_width - v.label.length() + 1) << ' ';
...
void dawg::output::print_nexus(const alignment& aln) {
    ...
    static constexpr char datatypes[][10] = {"DNA", "RNA", "PROTEIN", "DNA",
                                             "DNA", "RNA", "PROTEIN", "DNA"};

    // TODO: Move this to the block support routine
    out << "#NEXUS\n[Created by " DAWG_PACKAGE_STRING "]" << endl;

    out << "BEGIN DATA;\n"
           "\tDIMENSIONS NTAX="
        << aln.size() << " NCHAR=" << aln[0].seq.size()
        << ";\n"
           "\tFORMAT DATATYPE=";
